...
class TestRailPlugin():
        ...
        if idmarker:
            ...
            if rep.when == 'call' and tid:
                ...
                if (rep.skipped and xfail) or (rep.failed and not xfail):
                    ...
                    self.add_result(tid, outcome.result.outcome, comments)
                else:
                    self.add_result(tid, outcome.result.outcome, comments="This result was marked by automation")

    @pytest.hookimpl(tryfirst=True)
    def pytest_runtest_setup(self, item):
        """ Plugin deals with testrail
            so if no id or id isn't in test case, skip
        """
        idmarker = item.get_closest_marker('testrail')
        if idmarker is None:
            pytest.skip('skip')
    ...
    def add_result(self, tid, status, comments):
